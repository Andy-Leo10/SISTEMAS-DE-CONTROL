...
double k1, k2, k3, k4, g1;
volatile double x1, x2, x3, x4, sp, u_control;
...
void loop()
{
  ...
  x4 = velocidad_inclinacion();

  u_control = g1 * sp - (k1 * x1 + k2 * x2 + k3 * x3 + k4 * x4);
  Serial.print("u_control: ");
  Serial.print(u_control);
  u_control = saturador(u_control, -5.7, 5.7);
