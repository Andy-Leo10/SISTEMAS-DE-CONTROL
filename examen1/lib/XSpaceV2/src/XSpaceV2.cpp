...
bool XSpaceV2Board::sw1_presionado()
{
    bool estado = 0;
...
bool XSpaceV2Board::sw2_presionado()
{
    bool estado = 0;
...
double XSpaceV2Board::temperatura()
{

    double lectura = 0;
    double valor = 0;
    lectura = analogRead(tempPin);
    valor = (lectura / 1023) * 300;
    // no necesita filtro porque es lento el cambio
    return valor;
}
...
void XSpaceV2Board::Mqtt_init(const char *mqtt_server, uint16_t mqtt_port)
{
    ...
    XSpaceV2MQTT.setCallback([this](char *topic, byte *payload, unsigned int length)
...
// del examen
double XSpaceControl::control_temperatura(double temp, double temp_ref, double coef1, double coef2)
{
    double u = 0;
    double e = temp_ref - temp;

    u = this->_u_ant + coef1 * e - coef2 * this->_e_ant + 50;
